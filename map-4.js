document.addEventListener('DOMContentLoaded', () => {
  ...
  // A plain, non-clustering layer swapped in while the Timeline panel is
  // active - clustering is great for normal browsing, but cluster bubbles
  // reflowing/resizing on every tick works against what a timeline is
  // supposed to show.
  let timelineActive = false;
  window.setTimelineActive = function (active) {
    if (active === timelineActive) return;
    map.removeLayer(timelineActive ? plainMarkers : markerCluster);
    timelineActive = active;
    map.addLayer(activeLayer());
    applyFilters();
  };
  function activeLayer() {
    return timelineActive ? plainMarkers : markerCluster;
  }
...
function applyFilters() {
  ...
  if (timelineActive) {
    visible.forEach(m => plainMarkers.addLayer(m));
  } else {
